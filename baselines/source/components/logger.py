...
def logger_factory(config: DictConfig) -> Tuple[logging.Logger, Path]:
    # log_path = Path(config.log_path) / config.unique_id
    log_path = Path(config.unique_id)
    
    log_path.mkdir(exist_ok=True, parents=True)
    logger = set_file_handler(log_file_path=log_path / config.unique_id)
